...
import { getServerJwt } from "@/lib/utils/getServerJwt";
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { name: product.title, images: product.imageUrls?.[0] ? [product.imageUrls[0]] : [] },
...
export const verifyPaymentAndCreateOrder = async (sessionId) => {
  try {
    const token = await getServerJwt(); // ✅ buyer এর JWT — এখন থেকে এটাই attach হবে সব protected কলে
    ...
    // ✅ Fix — এখন token attach হচ্ছে (verifyRole('buyer') পাস করবে)
    ...
    // ✅ Fix — আলাদা stock-decrement endpoint, role-conflict এড়ানোর জন্য
    if (product?.stockQuantity != null) {
      await safeFetchJson(`${apiBaseUrl}/api/products/${productId}/stock`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...authHeader },
        body: JSON.stringify({ decrementBy: qty }),
      });
    }
